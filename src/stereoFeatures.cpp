...
LineFeature* LineFeature::safeCopy(){
    ...
    Matrix3d LineFeature::skew(Vector3d v){
...
void LineFeature::getCov3DStereo(cv::Mat K, float baseline)
{
    ...
    Matrix3d jacMat;
    getStereoJacob3D_2D(spl(0),spl(1),sdisp,baseline,fx,cx,cy,jacMat);
    //求得相机坐标系的坐标协方差矩阵
    ...
    cov2D(1, 1) = pow( epl_std, 2 );
    cov2D(2, 2) = pow( sdisp_std, 2 );
    getStereoJacob3D_2D(epl(0),epl(1),edisp,baseline,fx,cx,cy,jacMat);
    //求得相机坐标系的坐标协方差矩阵
    covEpt3D = jacMat * cov2D * jacMat.transpose();
}

    void LineFeature::getStereoJacob3D_2D(const double &u, const double &v, const double &d,double b,double f,double cx,double cy,Matrix3d & jacMat) {
        double d_2  = d * d;
        //
        jacMat(0, 0) = b/d;
        jacMat(1, 0) = 0.0f;
