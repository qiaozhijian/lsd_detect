...
Config::Config()
{
    ...
    has_robot          = false;      // true if using IMU and Odometer
    close_loop         = true;      // true if Config::closeLoop()
    speed_up           = 1;
...
void Config::loadFromFile( const string &config_file )
{
    ...
    Config::hasRobot() = loadSafe(config, "has_robot", Config::hasRobot());
    Config::closeLoop() = loadSafe(config, "close_loop", Config::closeLoop());
    ...
    Config::speedUp() = loadSafe(config, "speed_up", Config::speedUp());
